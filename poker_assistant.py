...
class PokerAssistant:
    ...
    def AnalyzeAI(self, hero_buttons_map, poker_game_data):
        ...
        gpt4_output = self.analyze_game_state_with_gpt4(
            hero_buttons_map, poker_game_data
        )
        ...
        if gpt4_output is not None:
            ...
            return self.extract_hero_action_details_from_gpt4_output(
                hero_buttons_map, gpt4_output
            )

        else:
            print(
                f"{Fore.RED}Failed to get response in 26 seconds, so I will FOLD the hand now.."
            )
    ...
    def analyze_game_state_with_gpt4(self, hero_buttons_active, realtime_game_data):
        try:
            ...
            available_actions = "\n".join(
                [f"- {info['action']}" for info in hero_buttons_active.values()]
            )
            ...
            print("-------------------------------------------------------------------")
            print(
                f"analyze_game_state_with_gpt4() -> Time taken: {end_time - start_time} seconds"
    ...
    def extract_hero_action_details_from_gpt4_output(
        self, hero_buttons_map, gpt4_output
    ):
        """
    ...
    def analyze_players_gpt4(self, historical_data):
        try:
            ...
            if len(formatted_data) == 0:
                print(
                    f"{Fore.RED}Error: analyze_players_gpt4() historical_data is NULL. Returning..."
                )
                return
            ...
            print("-------------------------------------------------------------------")
            print(
                f"analyze_players_gpt4() -> Time taken: {end_time - start_time} seconds"
            )  # Calculate and print the time taken
            print("-------------------------------------------------------------------")
            # Extract the GPT-4 response
            gpt_response = response.choices[0].message.content
        ...
        for player in data["players"]:
            ...
            if 1 <= player_number <= 6:
                self.game_state.update_player(
                    ...
                    exploitation_strategy=exploitation_strategy_str,
                )
            else:
                print(
                    f"{Fore.YELLOW}Invalid player number: {player_number}. Skipped updating."
                )
