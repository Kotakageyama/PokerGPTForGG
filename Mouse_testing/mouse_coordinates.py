...
class MouseCoordinates:
    ...
    def locate_poker_window(self):
        ...
        windows = pwc.getWindowsWithTitle("No Limit")
        for window in windows:
            if "USD" in window.title:
                self.window = window
                print(
                    f"Poker client window found. Size: {self.window.width}x{self.window.height}"
                )
                return
    ...
    def run_mouse_listener_in_thread(self):
        ...
        listener_thread = threading.Thread(
            target=self.start_mouse_listener, daemon=True
        )
    ...
    def display_mouse_relative_position(self):
        ...
        try:
            ...
            while True:
                ...
                print(
                    f"Relative Mouse Position: x={relative_x:.3f}, y={relative_y:.3f}",
                    end="\r",
                )
